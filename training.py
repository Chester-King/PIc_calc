#python training.py --dataset images --model calc.model
import matplotlib
matplotlib.use("Agg")
...
def test():
    ...
    for rect in rects:
        ...
        if nbr>9:
            nbr = dic[nbr[0]]
        pre.append(nbr[0])
        cv2.putText(im_gray, str(nbr[0]), (rect[0], rect[1]),cv2.FONT_HERSHEY_DUPLEX,4,(0,255,255),2,cv2.LINE_AA)
    ...
    #print(predictions)
    cv2.imwrite('e_edited.jpg',im_gray)
#train()
test()